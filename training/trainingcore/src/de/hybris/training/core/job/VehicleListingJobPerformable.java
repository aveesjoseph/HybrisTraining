...
public class VehicleListingJobPerformable extends AbstractJobPerformable<CronJobModel> {
    ...
    public PerformResult perform(CronJobModel cronJobModel) {
        ...
        for(int i=0;i<100;i++ ){
            ...
            if(clearAbortRequestedIfNeeded(cronJobModel)){
                LOG.info("Vehicle Cronjob is Aborted at count = "+i);
    ...
    private void printSampleCronJobCount(int i) {
        try {
            LOG.info("Printing Vehilce Cronjob where count = "+i);
            Thread.sleep(500);
