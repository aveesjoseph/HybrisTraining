 ...
 * --- Generated at 09-Jul-2021, 1:43:49 PM                     ---
 * ----------------------------------------------------------------
 *  
...
public abstract class GeneratedTrainingCoreManager extends Extension
{
	...
	public TrainingEmployee createTrainingEmployee(final SessionContext ctx, final Map attributeValues)
	{
		try
		{
			ComposedType type = getTenant().getJaloConnection().getTypeManager().getComposedType( TrainingCoreConstants.TC.TRAININGEMPLOYEE );
			return (TrainingEmployee)type.newInstance( ctx, attributeValues );
		}
		catch( JaloGenericCreationException e)
		{
			final Throwable cause = e.getCause();
		...
		{
			throw new JaloSystemException( e ,"error creating TrainingEmployee : "+e.getMessage(), 0 );
		}
	}
	
	public TrainingEmployee createTrainingEmployee(final Map attributeValues)
	{
		return createTrainingEmployee( getSession().getSessionContext(), attributeValues );
	}
